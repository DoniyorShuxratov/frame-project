...
export default function HomePage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-ds-6 lg:px-ds-8 py-ds-6 sm:py-ds-10">
      ...
      {/* Banner */}
      <Link href="/products" className="block mb-ds-6 sm:mb-ds-10 rounded-md sm:rounded-xl overflow-hidden">
      ...
      {/* Header */}
      <div className="flex items-center justify-between mb-ds-4 sm:mb-ds-8">
        <div>
          <h1 className="font-gilroy font-bold text-h3 sm:text-h1 text-content-primary">Shop</h1>
      ...
      {/* Category filters */}
      <div className="flex flex-wrap gap-1.5 mb-ds-4 sm:mb-ds-8">
        {categories.map((cat) => (
          <button
            key={cat}
            onClick={() => setCategory(cat)}
            className={[
              "px-3 sm:px-ds-4 py-1 sm:py-ds-1.5 font-gilroy font-semibold text-xs sm:text-small uppercase tracking-widest rounded-full border transition-all",
              category === cat
      ...
      {/* Grid */}
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 sm:gap-ds-6">
            ...
            <Link key={product.id} href={`/products/${product.id}`} className="group block">
              <div className="bg-surface-card border border-stroke-default rounded-md sm:rounded-xl overflow-hidden transition-colors">
                ...
                <div className="p-2 sm:p-ds-4">
                  <p className="text-xs text-content-secondary font-gilroy uppercase tracking-widest mb-0.5 sm:mb-1 truncate">
                    {product.category}
                  </p>
                  <h3 className="text-xs sm:text-body font-gilroy font-semibold text-content-primary leading-snug mb-1 sm:mb-2 line-clamp-2">
                    {product.name}
                  </h3>
                  <span className="text-small sm:text-h5 font-gilroy font-bold text-content-primary">
